...
async function getCountryByName(name: string): Promise<Country> {
    ...
    //!no final garante que eu vá encontrare não preciso tipar undefined
    ...
    return (
        <section className="flex flex-col container">
            ...
            <article className="flex md:flex-row flex-col justify-between min-w-full p-10 bg-white rounded-xl">
                <section>
                    {country?.capital?.length && (<h2 className="text-xl text-gray-800 mt-3">
                    ...
                    {country?.languages && (<h2 className="text-xl text-gray-800 mt-3">
                        <b> 🗣️ Língua faladas:</b>
                        <br />
                        {Object.values(country.languages).map((language) => (
                            <span key={language} className="inline-block px-2 bg-indigo-500 mr-2 text-white text-sm rounded-full">
                                {language}
                            </span>
                        ))}
                    </h2>)}
                ...
                <div className="relative h-32  my-2 md:h-auto w-42 md:w-96 shadow-md md:order-last order-first">
                    <Image
            ...
            <section>
                <h3 className="mt-12 text-2lx font-semibold text-gray-800">
                    Paises que fazem fronteiras </h3>
                <div className="container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 
    lg:grid-cols-4 xl:grid-cols-5 gap-2 w-full">
